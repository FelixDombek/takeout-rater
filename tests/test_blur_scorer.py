"""Tests for the BlurScorer heuristic."""
...
def test_score_batch_real_image(tmp_path: Path) -> None:
    ...
    # Create a simple gradient image (has edges, should be non-zero sharpness)
...
def test_score_batch_uniform_image_lower_than_gradient(tmp_path: Path) -> None:
    ...
    # Uniform (all grey) → should have low sharpness
    Image.new("RGB", (64, 64), color=(128, 128, 128)).save(uniform, "JPEG")

    # Checkerboard (high contrast) → should have high sharpness
    img = Image.new("RGB", (64, 64))
    for x in range(64):
        for y in range(64):
