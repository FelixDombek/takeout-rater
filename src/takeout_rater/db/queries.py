...
# ---------------------------------------------------------------------------
# Allowed columns for safe dynamic UPDATE statements
# ---------------------------------------------------------------------------

_ALLOWED_METADATA_COLS: frozenset[str] = frozenset(
    {
        "sha256",
        "title",
        "description",
...
def update_asset_metadata(
    conn: sqlite3.Connection, asset_id: int, updates: dict[str, Any]
) -> None:
    """Apply a dict of column→value updates to one asset row.

    Only columns in :data:`_ALLOWED_METADATA_COLS` are accepted; any other
    ...
    """
    safe = {k: v for k, v in updates.items() if k in _ALLOWED_METADATA_COLS}
    if not safe:
        return
    set_clause = ", ".join(f"{k} = ?" for k in safe)
    conn.execute(
        f"UPDATE assets SET {set_clause} WHERE id = ?",  # noqa: S608
        [*safe.values(), asset_id],
    )


def dedup_assets_by_sha256(conn: sqlite3.Connection) -> int:
    """Merge exact duplicate assets (same SHA-256) into the ``asset_paths`` table.

    Keeps the asset row with the lowest ``id`` as canonical and moves all
    secondary relpaths into ``asset_paths``.  Assets that have associated
    scores or phash entries are skipped to avoid cascading deletions.

    Only processes assets that have a non-NULL ``sha256`` and share it with at
    least one other asset row (i.e. newly inserted duplicates).

    Args:
        conn: Open database connection.

    Returns:
        The number of duplicate asset rows merged into ``asset_paths``.
    """
    # Find assets whose sha256 is shared with an asset that has a lower id.
    duplicates = conn.execute(
        """
        SELECT a.id, a.relpath, a.sha256
        FROM assets a
        WHERE a.sha256 IS NOT NULL
          AND EXISTS (
            SELECT 1 FROM assets b
            WHERE b.sha256 = a.sha256 AND b.id < a.id
          )
        ORDER BY a.id
        """
    ).fetchall()
    ...
    for dup_id, relpath, sha256 in duplicates:
        # Skip if this asset already has scores or phash (safety guard).
        has_data = conn.execute(
            "SELECT 1 FROM asset_scores WHERE asset_id = ? LIMIT 1",
            (dup_id,),
        ).fetchone()
        if has_data:
            continue
        ...
        canonical = conn.execute(
            "SELECT id FROM assets WHERE sha256 = ? AND id < ? ORDER BY id LIMIT 1",
            (sha256, dup_id),
        ).fetchone()
        ...
        conn.execute(
            ...
            " ON CONFLICT(relpath) DO UPDATE"
            "   SET asset_id = excluded.asset_id, indexed_at = excluded.indexed_at",
            (canonical_id, relpath, now),
        )
        conn.execute("DELETE FROM assets WHERE id = ?", (dup_id,))
        merged += 1
